    ...
    try:
        with uow:
            ...
            data_dict = {
                ...
                'image_url': record.image_url,
                'pokemon_url': record.pokemon_url,
                'abilities': ', '.join([ability.name for ability in record.abilities or []]),
                'stats': '; '.join([f"{stat.name}: {stat.base_stat}" for stat in record.stats or []]),
                'types': ', '.join([type.name for type in record.types or []]),
            }

            print("Data dictionary for CSV:", data_dict)

            # Convert to DataFrame
            data_df = pd.DataFrame([data_dict])  # Wrap in a list to create a valid DataFrame
